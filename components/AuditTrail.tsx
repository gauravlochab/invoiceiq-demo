  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <p className="section-label m-0">Audit Trail</p>
        <span className="text-[9px] text-[var(--text-muted)]">{entries.length} entries</span>
      </div>

      <div className="flex flex-col gap-0">
        {visibleEntries.map((entry, i) => {
          const Icon = actionIcons[entry.action] || Eye;
          const dotColor = actorTypeColors[entry.actorType] || "var(--text-tertiary)";
            ...
            <div key={entry.id} className="flex gap-2.5">
              ...
              <div className="pb-3 flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                  ...
                  <span className="text-[10px] font-semibold text-[var(--text-primary)]">{entry.actor}</span>
                  <span
      ...
      {entries.length > defaultVisible && (
        <button
          ...
          className="flex items-center gap-1 text-[10px] text-[var(--acl-primary)] font-medium mt-1 cursor-pointer bg-transparent border-none hover:underline p-0"
        >
          {showAll ? (
            <>
